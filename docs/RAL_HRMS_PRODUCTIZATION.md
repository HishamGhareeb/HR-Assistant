# RAL HRMS Productization Plan

This track turns RAL HRMS from an upstream HR operations system into one backend component inside a RAL Technologies commercial HRMS product.
...
RAL AI, compliance, payroll, tenancy, billing, and audit layer
        ↓
RAL HRMS operational records and workflows
```

RAL HRMS is the engine room. RAL is the product.

## Implementation levels

### Level 1 — RAL-branded RAL HRMS
...
### Level 2 — RAL product layer on RAL HRMS

- Dedicated `ral_hrms` RAL HRMS app.
...
- RAL HRMS hooks that sync HR records into the HR Assistant retrieval and authorization systems.
- OpenFGA role synchronization from tenant-scoped RAL HRMS roles.
- Deterministic Bahrain-first country-law packs.

### Level 3 — RAL HRMS as hidden backend

- Customer-facing RAL employee, manager, HR admin, payroll, and tenant admin portals.
- RAL HRMS Desk limited to internal operators or advanced system administrators.
- RAL APIs become the stable product contract.

## First productization slice

The first implementation slice is a scaffold and contract, not a full RAL HRMS bench app:

- `glue.ral_hrms_productization` defines the canonical RAL productization metadata.
- `integrations/ral_hrms/ral_hrms` contains the planned RAL HRMS app structure.
...
## RAL HRMS customization strategy

Use RAL HRMS's intended extension mechanisms:
...
Avoid editing RAL HRMS core unless a hook/app extension cannot solve the problem.
...
RAL HRMS remains the operational source for HR records. RAL product entities provide tenancy, AI, compliance, audit, payroll localization, billing, and product control.
...
## RAL HRMS-to-AI sync hooks

The RAL HRMS app should eventually emit tenant-scoped sync events for:
...
- RAL HRMS mutations require separate explicit workflows.
...
2. Add durable tenant provisioning and role sync from RAL HRMS/RAL tenants into OpenFGA.
...
5. Build the first RAL frontend API endpoints that hide raw RAL HRMS surfaces from normal users.
