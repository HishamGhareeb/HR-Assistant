...
{"query": "<question text>", "filters": {"document_set": ["tenant:<tenant_id>"]}}
```
...
tenant's document set (`document_set: ["tenant:<id>"]`) so one tenant's data
is less likely to even be considered as a retrieval candidate for another
tenant's question.
...
and can be upgraded to the fully-validated `glue.domain.Document` /
`Citation` contract via `.to_canonical()`.
...
Frappe → Onyx sync. Same for `metadata.tenant_id`.
