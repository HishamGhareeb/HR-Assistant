...
`glue/hr_source_sync.py` is the deterministic path from a RAL HRMS record to
...
real RAL HRMS REST/webhook source is out of scope here; it would only need
to produce the same `HrSourceRecord` shape this module already consumes —
see "Real RAL HRMS integration" below for exactly what that means.

## Shape

- `HrSourceRecord(doctype, name, tenant_id, fields, deleted)` — the generic
  stand-in for "one row RAL HRMS's API returned for one doctype."
- `map_record(record, config) -> MappingResult` — a **pure function**: one
  ...
  which is what makes "what does this RAL HRMS record mean" trivially unit
...
| HR source record type     | Retrieval document    | Tuples contributed                                                                 |
|---------------------|------------------------|--------------------------------------------------------------------------------------|
...
`SyncConfig.hr_admin_user_ids` is sync-run configuration, not a RAL HRMS
field — RAL HRMS doesn't tag "who is HR admin" per record, so treating it as
...
`HrSourceMappingError` for that one record — `SyncEngine` catches this per
...
enforced structurally, not by a background sweep: a `HrSourceRecord` with
...
pass to fall behind or be skipped. A real RAL HRMS source needs to surface
deletions/revocations (e.g. a RAL HRMS "on_trash" hook or a status field
flip) as a `HrSourceRecord(deleted=True)` for this to hold in practice; that
wiring is part of "Real RAL HRMS integration" below, not this module.

## Real RAL HRMS integration (not in this ticket)

Everything above operates on the generic `HrSourceRecord` shape. Wiring a
real RAL HRMS instance in means building a source that:

1. Polls or receives webhooks for the six doctypes above (RAL HRMS's REST
   API returns exactly `doctype` + `name` + field dict already).
2. Maps RAL HRMS's `user_id`/`reports_to`/`department` link fields to the
   same field names this module expects (`user_id`, `department`,
   `reports_to`, `employee_user_id`).
3. Emits a `HrSourceRecord(deleted=True)` on delete/on_trash/status-revoked,
   not just on create/update.

No change to `map_record` or `SyncEngine` should be required — that's the
point of keeping `HrSourceRecord` a plain, source-agnostic shape.
