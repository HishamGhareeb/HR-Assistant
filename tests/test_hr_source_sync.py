"""Synthetic end-to-end tests for the RAL HRMS -> Onyx/OpenFGA sync, per
HIS-15's acceptance criteria ("Synthetic end-to-end tests only"). No live
RAL HRMS, Onyx, or OpenFGA -- FakeDocumentIndex/FakeTupleWriter stand in for
...
from glue.hr_source_sync import (
    HrSourceMappingError,
    HrSourceRecord,
...
def test_map_employee_missing_user_id_raises():
    bad = HrSourceRecord(doctype="Employee", name="EMP-x", tenant_id="acme", fields={"department": "engineering"})
    with pytest.raises(HrSourceMappingError):
        map_record(bad, SyncConfig())


def test_map_department_produces_no_document_or_tuples():
    department = HrSourceRecord(doctype="Department", name="engineering", tenant_id="acme", fields={})
...
def test_map_record_rejects_unsupported_doctype():
    weird = HrSourceRecord(doctype="Something Else", name="x", tenant_id="acme", fields={})
    with pytest.raises(HrSourceMappingError):
...
async def test_deleted_record_is_retracted_from_index_and_tuples():
    ...
    deleted_record = HrSourceRecord(doctype="Leave Application", name="LA-1", tenant_id="acme", deleted=True)
...
async def test_one_failed_record_does_not_block_the_rest_of_the_batch():
    ...
    bad = HrSourceRecord(doctype="Employee", name="EMP-bad", tenant_id="acme", fields={})  # missing user_id
