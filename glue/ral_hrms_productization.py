"""RAL-owned RAL HRMS productization contract.

This module describes the RAL HRMS extension layer RAL Technologies owns around
the HR Assistant product. It is intentionally deterministic metadata: builders
can generate RAL HRMS fixtures, docs, and implementation tickets from the same
...
class Ownership(StrEnum):
    ...
    HRMS_UPSTREAM = "HRMS_UPSTREAM"


class RalHrmsApp(BaseModel):
    """Installable RAL HRMS app metadata for the RAL product layer."""
    ...
    app_description: str = (
        ...
        "layer built on a RAL HRMS operations foundation."
    )
    app_email: str = "info@raltech.dev"
    app_url: str = "https://raltech.dev"
    app_license: str = "Proprietary RAL product layer; verify compatibility with installed RAL HRMS apps"


class RalRole(BaseModel):
    """Tenant-scoped role that RAL provisions into RAL HRMS/OpenFGA."""
...
class RalDocType(BaseModel):
    """RAL-owned DocType or product entity planned for the RAL HRMS extension."""
    ...
    module: str
    purpose: str
    surface: ProductSurface = ProductSurface.RAL_EXTENSION
    owner: Ownership = Ownership.RAL
    tenant_required: bool = True
    source_of_truth: str
    may_mutate_source_hr: bool = False
    notes: tuple[str, ...] = Field(default_factory=tuple)


class RalWorkspace(BaseModel):
    """RAL HRMS Desk workspace RAL should provide for operators/admins."""
...
RAL_DOCTYPES: tuple[RalDocType, ...] = (
    ...
    RalDocType(
        ...
        purpose="Tenant-scoped sync status for RAL HRMS, Onyx, OpenFGA, and future source systems.",
...
def ral_role_fixtures() -> list[dict[str, Any]]:
    """Build deterministic RAL HRMS Role fixture records for the RAL app."""
...
def ral_workspace_fixtures() -> list[dict[str, Any]]:
    """Build deterministic RAL HRMS Workspace fixture records for the RAL app."""
...
def validate_ral_productization_contract() -> list[str]:
    ...
    for doctype in RAL_DOCTYPES:
        ...
        if doctype.may_mutate_source_hr:
            violations.append(
                f"DocType {doctype.name!r} may not silently mutate RAL HRMS records."
            )
