"""Deterministic sync from RAL HRMS records into Onyx retrieval documents
and OpenFGA authorization tuples.

## Shape

`HrSourceRecord` is the generic, synthetic stand-in for "one row RAL HRMS's
REST API returned for one doctype" -- this ticket is scoped to *synthetic*
data (see docs/hr_source_sync.md); a real RAL HRMS polling/webhook source is a
later concern and would just need to produce the same `HrSourceRecord`
shape.

`map_record` is a pure function: one `HrSourceRecord` in, one `MappingResult`
...
that keeps the "what does this RAL HRMS record mean" logic trivially unit
...
Every `HrSourceRecord.tenant_id` flows straight into the document metadata
...
# --- RAL HRMS input shape -----------------------------------------------------


@dataclass(frozen=True)
class HrSourceRecord:
    """One doctype row as RAL HRMS's REST API would return it. `deleted=True`
...
class HrSourceMappingError(ValueError):
    """A HrSourceRecord's `doctype` is unsupported, or a required field for
    its doctype is missing -- raised rather than guessing, so a malformed
    synthetic (or eventually real) RAL HRMS row surfaces as a sync failure
...
@dataclass
class SyncConfig:
    """Sync-run-wide policy that isn't itself a RAL HRMS field: which users
    hold the hr_admin role. RAL HRMS doesn't tag this per-record, and
    treating it as sync configuration (rather than inventing a synthetic
    RAL HRMS field for it) keeps `map_record` honest about what actually
    comes from RAL HRMS."""
...
def _require(fields: dict, key: str, doctype: str) -> str:
    ...
    if not isinstance(value, str) or not value.strip():
        raise HrSourceMappingError(f"{doctype} record is missing required field {key!r}")
    return value.strip()


def _map_employee(record: HrSourceRecord) -> MappingResult:
...
def _map_department(record: HrSourceRecord) -> MappingResult:
...
def _map_leave_application(record: HrSourceRecord, config: SyncConfig) -> MappingResult:
    fields = record.fields
...
def _map_appraisal(record: HrSourceRecord, config: SyncConfig) -> MappingResult:
    fields = record.fields
    employee_user_id = _require(fields, "employee_user_id", record.doctype)
    department = _require(fields, "department", record.doctype)
...
def _map_salary_slip(record: HrSourceRecord, config: SyncConfig) -> MappingResult:
    fields = record.fields
    employee_user_id = _require(fields, "employee_user_id", record.doctype)
...
def _map_hr_policy(record: HrSourceRecord) -> MappingResult:
    fields = record.fields
...
def map_record(record: HrSourceRecord, config: SyncConfig) -> MappingResult:
    mapper = _MAPPERS.get(record.doctype)
    if mapper is None:
        raise HrSourceMappingError(f"unsupported HR source record type: {record.doctype!r}")
    return mapper(record, config)


def document_object_id(record: HrSourceRecord, document: IndexedDocumentRef) -> str:
    ...
    return f"RAL HRMS:{record.tenant_id}:{document.object_type.value}:{record.name}"
...
class SyncEngine:
    ...
    async def sync_record(self, record: HrSourceRecord, report: ReconciliationReport) -> None:
        ...
        except HrSourceMappingError as exc:
            report.failed.append(RecordFailure(record.doctype, record.name, str(exc)))
            return
    ...
    async def sync_all(self, tenant_id: str, records: list[HrSourceRecord]) -> ReconciliationReport:
